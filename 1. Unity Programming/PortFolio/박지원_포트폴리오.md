...
### 1️⃣ Level_0 ~ Level_2  =>  대학 / 첫 취업 준비 / 직장 재직 시기
#### 검색하고 참고하되, 최종 코드는 직접 구현했습니다.

- ![Level 0~2 경험 이미지](./captures/ai-level-0-2.png)

- 대학과 취업 준비 시기에는 검색, StackOverflow, 자동완성을 참고하며 직접 코드를 작성했습니다.
- 넥슨에서는 시니어 개발자분들의 코드와 사내 자료를 접했지만, 이해하지 못한 코드를 외워서 사용하면 제 구현력으로 남지 않는다는 한계도 경험했습니다.
- C++ 코딩테스트 지식이 Unity/C# 개발과 분리되어 있다고 느껴, 퇴사 이후에는 C#으로 다시 문제를 풀며 컬렉션·문자열·할당·복사 비용을 공부했습니다.
- 이 시기의 직접 구현 경험은 이후 AI가 작성한 코드를 판단하는 기준이 되었습니다.
...
### 2️⃣ Level_3  =>  직장 재직 시기 / 퇴사 이후 2025년
#### GPT를 답을 얻는 도구가 아니라, 이해를 넓히는 도구로 사용했습니다.
...
### 4️⃣ 앞 단계의 경험은 사라진 것이 아니라, 현재의 판단 기준이 되었습니다.

- 예전에는 코드를 직접 작성하는 것 자체가 구현력이라고 생각했습니다.
- 지금은 문제와 제약을 정의하고, AI가 작성한 결과를 검증하고, 잘못된 방향을 수정하는 능력까지 구현력의 범위라고 생각합니다.
- AI 없이 직접 구현할 수 있는 기본기를 유지하면서, AI를 통해 더 넓은 범위의 개발을 빠르게 수행하는 방식을 만들어가고 있습니다.
