...
">
  ...
  <div style="
    ...
    line-height: 1.6;
  ">
    Notion과 TFT에서, 기록을 반복해서 다듬고 다시 볼수록 <br>
    저만의 판단 기준이 생긴다는 것을 경험했습니다.
  </div>
...
### 1️⃣ 직장에서 해결하지 못했던 '회사에서 공부하기'

- 회사는 공부하는 공간이 아니다. 그러나 업무 과정에서 모르는 건 필연적으로 생기고 공부는 해야한다.

- 메모를 하고 -> 메모를 다시 공부하고 -> 기록하고 -> 다시 보며 업데이트 과정 -> 이 프로세스가 결국 내가 생각하는 공부의 왕도고 전 직장에서는 결국 해내지 못했다.

- 근데 이 과정은 직장인이 현실적으로 하기에는 벅찼다. 

- 그러나 이번에 Notion -> 롤토체스 -> Unity Client 개발에서 이뤄냈다.

<div style="page-break-after: always;"></div>

# 📚 [세 번째 해결과정] <br> 제목 아직

###  2️⃣Notion(=메모 프로그램) : 결국에는 하나의 문서를 계속 다시보고 다시보고 업데이트 하며 수십번 읽으면 내것이 되는 걸 깨닫게 해준 과정
...
- **2023년 회사 생활**
  - 결국 길면 다시 안 봄
  - 이렇게 길고 다시 보지 않은 메모가 수백개 있었다.
  - 글이 길고 장황하다는 건 메모를 가공하지 않았다는 듯.

<br>

- **현재**
  - 현재 사용하고 있는 월간 계획표 템플릿 형식으로 당시의 메모를 가공
  - 월간 계획표에 적어서 매일 아침 볼 수 있게 함
  - 계속 다시보며 고치고 고치고 고치고 줄임
...
###  3️⃣롤토체스(=게임)  :  나만의 기록을 통해 결과를 낸 사례
...
- 롤체에서 정보 수집 → 패턴 발견 → 판단 → 결과 검토 → 수정을 반복했고, 개발에서도 같은 방식이 필요하다고 깨달음
- TFT ↔ Unity 이미지
- 롤체 자체를 설명하지 않기. 롤체를 몰라도 이해돼야 함
...
<div style="text-align: center;">
  <img src="./captures/myPattern.png" width="600">
</div>
...
[2시간 업무의 과정]
개발 1시간이라고 가정하면
codex와 질의 응답 -> CODEX가 구현을 하되, 나는 거기서 내가 필요한 핵심 키워드와 질문 사항이 생긴다. -> 그걸 노션에 메모하고 -> 업무가 완료되면 codex가 자동화해서 세가지를 요청 키워드 질문사항 그리고 본인이 이해한 프롬프트 -> 나는 이걸 주면 codex가 markdown 초안을 생성 -> 나는 이걸 보고 집에 와서 공부한다. 이런 내용 적고

[그리고 이 기록은 개발 할 때 나의 설계 판단이 된다.]
