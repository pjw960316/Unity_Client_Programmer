...
### :one: 1. View와 1대1로 대응하고, View와 Presenter가 묶인 것을 VP라고 할 때 <br> 여러 개의 VP가 1개의 model을 참조하는 구조로 재사용을 구현하도록 한다.
...
### :two: Presenter -> View는 올바르다. 그러나 Presenter와 Connect된 View가 Popup이라고 가정할 때, Presenter는 그 Popup이 들고 있는 Widget(small view)에는 직접 접근하면 안 된다. 다시 말해, Presenter가 직접 Widget을 Setting하면 안 된다.
...
### :three: Presenter는 View 와 Model과 동등한 위치기 때문에 <br> Request를 메서드 이름 앞에 붙이지 않는다. 
