...
- ![alt text](./captures/20251016_3.png)
...
## :fire: 카메라에서 FOV는 <ins>각도</ins>다. <br> :fire:Tan(FOV / 2)는 화면 (절반의 높이 / 카메라와의 평면거리)를 의미한다. 
- 카메라가 해상도에 따라 화면을 보정할 때 이용되는 개념이다.
- Unity는 카메라의 FOV 계산에 실제 화면 크기(cm)를 사용하지 않고, 화면 비율과 픽셀 기준 렌더링 결과만 고려한다.
  - 화면 비율 : 해상도 1080 x 1920 이면 1080 / 1920을 의미한다.
