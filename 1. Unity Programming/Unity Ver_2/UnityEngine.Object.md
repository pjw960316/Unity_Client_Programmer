...
## :fire: Object가 unity null이 되면 unity 세상에서는 완전히 죽은 Instance(어떠한 기능도 할 수 없는) 취급이 된다. <br> :fire: 당연히 unity null 상태의 Object는 GC의 수집대상이다. <br> 그러나 C#에서 엄밀히 이야기 하면 null은 아니고, 이는 Unity가 C++로 구현되어 그렇다.
...
- **1번 규칙** ?.(=null-conditional operator) 그리고 ??(=null-coalescing operator) 사용하지 않기 
