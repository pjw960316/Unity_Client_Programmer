## :fireworks: 우선 Unity는 싱글 스레드기 때문에 메인 스레드와 서브 스레드는 사실 구분되지 않는다. <br> 하지만 설명을 위해 구분했다. <br> :fire: 메인 스레드는 await 키워드를 만나면 해당 메서드를 **즉시 탈출하고** 호출부로 돌아가서 이어서 진행한다. <br> 그리고 서브 스레드 1개가 이 시점부터 일을 시작한다. <br> :fire: 서브 스레드는 await 키워드에 걸린 Unitask를 비동기적으로 수행하고, 이를 완료하면 주 스레드에게 알린다. <br> :fire: 주 스레드는 완료 시점에 await 이후로 돌아와서 결과 값을 가지고 일을 수행한다.
#### [await 예제]
~~~c#
private void Test()
{
