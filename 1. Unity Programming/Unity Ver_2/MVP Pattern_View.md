...
- **Presenter가 MonoBehaviour를 통한 View 갱신을 요청 할 때, 그걸 줄 수 있는 Public Get Method**
...
- **Presenter를 들고 있지 않는다. Presenter -> View를 하도록 Presenter에게 자신가 자신을 이용하도록 한다.**
...
- **구현 순서  :  View Initialize() & Widget Initialize() -> View에서 Presenter 생성 -> Presenter는 생성되면서 Presenter Initialize() -> Presenter가 SetView()를 통해 Manager 또는 Model에서 받아온 Data로 View에 Inject하여 Data를 세팅한다.**
