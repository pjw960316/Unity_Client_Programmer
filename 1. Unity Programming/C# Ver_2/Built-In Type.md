...
- [ ] 인스턴스를 만들면 인스턴스 자체 주소는 스택에 저장되는데 인스턴스의 멤버의 주소는 어디에 저장되는 거임?
  - 중요 : 결국 testobj1의 주소만 stack에 저장하고 실제 데이터는 heap에 있음. 그러면 testobj1.value의 주소도 stack에 저장? 그건 아니다 어차피 testobj1의 주소만 알면 testobj1 가방 안에 있는 testobj1.value의 주소는 찾아 갈 수 있기 때문에 저장할 필요가 없다. 그래서 결과를 찍어보면 testobj1의 heap 주소(stack에 저장되어 있음)와 testobj1.value의 heap 주소는 별로 차이가 나지 않는 값이다.
  - 이거 잘 정리하자. 
