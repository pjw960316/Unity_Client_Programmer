...
> 다수의 string 객체가 하나의 문자열을 공유 할 수 있게 되어 시스템에서 필요로 하는 문자열의 개수를 줄여 메모리 사용량을 최소화할 수 있다. 
...
> C#에서는 String 객체를 만들기 위하여 리터럴 문자열을 생성자의 매개변수로 전달할 수 없다.
...
## :fire: [문자열 인터닝] <br> => 같은 문자열은 힙에 하나만 저장하고 해시테이블을 통해 하나만 존재하도록 관리한다.
~~~c#
void Main()
{
