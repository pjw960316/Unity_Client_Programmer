...
## :fireworks: Initialization에는 두 가지 방법이 있다. <br> :one: Field Initializer (=Initialization At Declaration) <br> :two: Constructor 

<br><br>

## :fire: Field initializer가 Constructor 보다 먼저 호출된다. <br> 그러므로, Constructor가 Field Initializer를 덮어 씌울 수 있다. <br> :fire: readonly Field는 두 가지 Initialization에서만 할당이 가능하다. <br> Unity의 Awake()나 Start()에서 불가능해서 불편하다.
