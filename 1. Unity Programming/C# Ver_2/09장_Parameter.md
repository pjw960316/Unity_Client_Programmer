...
- ref와 out은 값 타입을 참조로 전달하여 원본 값을 변경할 수 있도록 한다.
- ref는 호출 전에 초기화가 필요하므로 기존 값을 기반으로 수정할 때 사용한다.
- out은 호출 전에 초기화가 필요 없고 메서드 내부에서 반드시 값을 할당해야 하므로 새로운 값을 반환할 때 사용한다.