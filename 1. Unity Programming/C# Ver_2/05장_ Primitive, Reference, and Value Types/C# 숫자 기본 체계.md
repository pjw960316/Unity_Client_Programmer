...
## :fire: 서로 다른 타입 끼리 계산할 때 <br> C#은 더 범위가 넓은 타입을 리턴한다. <br> :fire: double이 long 보다 범위가 넓다. <br> :fire: 정수와 실수를 계산하면 실수로 캐스팅된다.
- **int + long => long 캐스팅**
...
- **int 와 long**
  - 100억은 int로 선언하면 오버플로우가 나는 걸 알기 때문에 long으로 선언한다.
  - 그러나 계산 과정에서 1억 * 100을 int에 초기화 하는 경우가 종종 있고, 이는 오버플로우가 된다.
