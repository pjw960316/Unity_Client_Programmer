...
## :two: 종이노트에 다음의 세 가지 항목을 적는다. <br> [1] 기능(메서드) 구분  <br> [2] 알맞은 자료구조 <br> [3] :star:자료구조를 구성할 DTO (데이터 클래스):star:
- 체감상 쉬운 문제는 메서드 1 ~ 2개로, 어려운 문제는 메서드 3개 이상으로 구성되는 것 같다.
- 메서드로 기능 분리와 알맞은 자료구조로 해결되는 문제는 난이도가 높아도 잘 푼다. <br> 그러나 DTO를 유니티의 concrete class처럼 필드를 상세히 만들어야 하는 문제는 잘 못 풀었다.
